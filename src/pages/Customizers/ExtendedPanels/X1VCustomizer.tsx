...
const X1VCustomizer: React.FC = () => {
  ...
  const [isHoveringLargeIcon, setIsHoveringLargeIcon] = useState(false);
  ...
  const handleDrop = (cellIndex: number, data: string) => {
    try {
      ...
      if (dragData.type === 'new') {
        ...
        // Check if trying to place icon in big icon area (position 100) - only allow socket icons
        if (cellIndex === 100 && icon.category !== "Sockets") {
        ...
        if (icon.category === "Sockets" && cellIndex !== 100) {
        ...
        // Check big icon area restrictions (position 100) - only allow socket icons
        if (cellIndex === 100 && sourceIcon.category !== "Sockets") {
          console.log("DROP: BLOCKED non-socket icon placement in big icon area");
          return;
        }
        if (dragData.position === 100 && targetIcon?.category !== "Sockets") {
          console.log("DROP: BLOCKED non-socket icon placement in big icon area");
          return;
        }

        // Check socket icon restrictions - only allow in big icon area (position 100)
        if (sourceIcon.category === "Sockets" && cellIndex !== 100) {
          console.log("DROP: BLOCKED socket icon placement in 3x3 grid - only allowed in big icon area");
          return;
        }
        if (targetIcon?.category === "Sockets" && dragData.position !== 100) {
          console.log("DROP: BLOCKED socket icon placement in 3x3 grid - only allowed in big icon area");
          return;
        }
  ...
  // Step-specific Big Icon Container Components
  const Step2BigIconContainer = ({ isReversed }: { isReversed: boolean }) => {
    const icon = placedIcons.find((i) => i.position === 100);
    return (
      <div style={{ width: '100%', height: '50%', minWidth: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 2 }}>
        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          <GridCell index={100} onDrop={handleDrop}>
            <div
              style={{
                width: '100%',
                height: '100%',
              ...
              onClick={() => {
                if (icon) return;
                // Optionally open icon picker or handle click
              }}
              onMouseEnter={() => setIsHoveringLargeIcon(true)}
              onMouseLeave={() => setIsHoveringLargeIcon(false)}
            >
              {icon ? (
                <img src={icon.src} alt={icon.label} style={{ width: '250%', height: '250%', objectFit: 'contain' }} />
              ) : (
        ...
        {/* Delete button positioned relative to the large icon container */}
        {placedIcons.find((i) => i.position === 100) && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              setPlacedIcons(prev => prev.filter(i => i.position !== 100));
            }}
            onMouseEnter={(e) => {
              e.stopPropagation();
              setIsHoveringLargeIcon(true);
              e.currentTarget.style.background = 'rgba(220, 53, 69, 1)';
              e.currentTarget.style.transform = 'scale(1.1)';
              e.currentTarget.style.boxShadow = '0 4px 12px rgba(220, 53, 69, 0.4)';
            }}
            ...
            style={{
              ...
              lineHeight: 1,
              zIndex: 10,
              opacity: isHoveringLargeIcon ? 1 : 0,
              transform: isHoveringLargeIcon ? 'scale(1)' : 'scale(0.8)',
              transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
              boxShadow: '0 2px 8px rgba(0, 0, 0, 0.2)',
              fontWeight: 'bold',
            }}
          ...
          {categoryIcons.map((icon) => (
            ...
            <div style={{ 
              ...
              <div
                style={{
                  width: "350px",
                  height: "700px", // reduced from 1000px by 30%
