          ...
          <Box sx={{ 
            ...
            <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5, color: '#1a1f2c', fontSize: '15px' }}>
              ...
              >
                ...
                {getBackboxOptions('DP').map((option) => (
...
const DPHCustomizer: React.FC = () => {
  ...
  const getPirIndex = (): number => {
    // Prefer first grid bottom-center (7), else second grid bottom-center (16)
  ...
  useEffect(() => {
    import("../../../assets/iconLibrary2").then((module) => {
      ...
      // Hide PIR category; control it via dedicated toggle like SPCustomizer
      setIconCategories(module.iconCategories.filter(cat => cat !== 'Sockets' && cat !== 'TAG' && cat !== 'Climate' && cat !== 'PIR'));
    });
  }, []);
  ...
  useEffect(() => {
    const checkMotionFlag = async () => {
      ...
      if (motionFlagData?.hasMotionFlag && !isEditMode && icons && Object.keys(icons).length > 0) {
        console.log('🔍 DPH Motion flag data received:', motionFlagData);
        ...
        try {
          ...
          if (pirIcon && !hasPIR) {
            ...
            setPlacedIcons(prev => [...prev, newPir]);
            console.log('✅ DPH PIR icon placed automatically due to motion flag');
          }
        } catch (error) {
  ...
  const { dimensions, iconLayout, textLayout, specialLayouts } = config;
              ...
              justifyContent: 'center',
                ...
                <div style={{ 
                  ...
                  style={{
                          ...
                          // Show all fonts when no search term
                          <>
        ...
        {currentStep === 2 && (
          <div style={{ marginBottom: "20px", display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div
              style={{
                    position: 'relative',
                width: typeof dimensions.width === 'string' && dimensions.width.endsWith('px') ? (parseInt(dimensions.width) * 2) + 'px' : '640px',
